...
public class PersonDAOImpl implements PersonDAO {
    ...
    public List<String> getSortedJobs() {
        return PersonDB.people.stream().map(Person::getJob).distinct().sorted().toList();
    }

    @Override
    public List<String> getSortedCities() {
        return PersonDB.people.stream().map(Person::getCity).distinct().sorted().toList();

    }

    @Override
    public List<String> femaleNames() {
       return PersonDB.people.stream().filter(person -> person.getGender().equals("Female")).map(Person::getFullName).toList();
    }

    @Override
    public Person highestEarner() {
        return PersonDB.people.stream().max(Comparator.comparingInt(Person::getSalary)).orElse(null);
    }
    ...
    public double averageSalary() {
        return PersonDB.people.stream().mapToDouble(Person::getSalary).average().getAsDouble();
    }
